...
--
-- The defaults of A6 are applied on READ (public.attention_config), and until
-- this trigger nothing looked at what was written. The UI validates a schedule
-- before saving it (frontend/src/utils/businessHours.ts), but an API key goes
-- straight to PostgREST and never sees that screen, and `extra` is a
-- free-form jsonb bag with no constraint on it.
--
-- The rules here are the UI's rules, deliberately: HH:MM, a window that
-- closes after it opens, and no two windows of a day over the same hour. Two
-- validators that disagree would be worse than one.
--
-- A trigger and not a CHECK because `extra` arrives as a JSON merge patch
-- (merge_update, on UPDATE only): the merged row is the only thing worth
-- validating, and a CHECK cannot see it. BEFORE triggers fire in alphabetical
-- order, so `validate_extra` runs after `set_extra` by name, not by luck.
create function public.validate_organization_attention() returns trigger
language plpgsql
set search_path to ''
as $$
declare
