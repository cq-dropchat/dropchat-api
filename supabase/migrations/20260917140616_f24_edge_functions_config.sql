...
CREATE OR REPLACE FUNCTION public.edge_functions_config(OUT url text, OUT token text)
 RETURNS record
 LANGUAGE plpgsql
...
-- Hand-written (db diff does not model privileges): it returns the service
-- token. Supabase's default privileges grant execute by name to anon,
-- authenticated and service_role; only the owner — the SECURITY DEFINER
-- triggers — calls it.
